...
{
    if (tree == NULL) // пока не пустой узел
        return;

    f(tree, arg); //  корень дерева печать
    apply_pre(tree->left, f, arg); // рекурсия левого поддерева
    apply_pre(tree->right, f, arg); // правого поддерева
}
...
void apply_bpre(Node_bt *tree, void (*bf)(Node_bt*, void*), void *arg)
{
    ...
    bf(tree, arg); //  корень дерева печать
    apply_bpre(tree->left, bf, arg); // рекурсия левого поддерева
    apply_bpre(tree->right, bf, arg); // правого поддерева
}
...
int hash1(int value, int size)//хеш функция
{
    return value % size;
}

int hash2(int value, int size)//2ая хеш функция
{
    return (float)size * ((C * (float)value) - (int)(C * (float)value));
}

int hashf(int value, char func, int size)//2 хеш функции в однрой в зав от func вызывает
{
    if (func == 1)
...
void compare_time(void)//изменить
{
